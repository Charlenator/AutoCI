...
| **B-evidence — Show underlying source rows in Citation Drawer** (NEW 2026-05-03 evening) | M | Today the SQL drawer shows the *aggregated* result row (`avg_days=83.3, sample_size=3`). For source traceability we should also show the **3 underlying hires** that produced that average. Implementation: each `SQLTemplate` gets an optional `build_evidence(params) -> str` that returns a non-aggregated SELECT for the underlying records. `ExecutorResult` gains `evidence_rows` (and `evidence_sql`). The Citation Drawer renders aggregate up top + an expandable "Source records (N)" table below. SQL itself stays visible. Closes the same brief requirement as the Citation Drawer but at a stronger level — reviewers see exactly which records produced the number. Pick up FIRST next session per Charle's request. |
