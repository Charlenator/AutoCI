...
>
> **Update — same evening, fourth pass**: Modal deployment unblocked. `backend/modal_config.py` rewritten end-to-end — single `autoci-secrets` Secret (was 6 phantoms), `pip_install_from_requirements` path corrected, `bge-small-en-v1.5` weights baked into the image at build time so cold starts skip the ~130 MB download, and a second Modal entry-point `drain_inbound_queue` scheduled every 2 minutes calling `inbound_processor.process_all_pending()`. Modal API surface verified against the installed modal 1.4.2.
>
> **Update — same evening, fifth pass — DEPLOY LIVE**: Backend running at `https://charlenator--autoci-backend-fastapi-app.modal.run`. /health 200 in 18.5s on cold start. /chat/query for "average time to fill for Java Developers" returned 83.3 days + 3 underlying hire records via B-evidence — full Modal → FastAPI → Supabase → DeepSeek → bge-small → SQL stack confirmed. Deploy required two extra iterations on `modal_config.py`: (a) Modal 1.4 deprecation fixes — `container_idle_timeout` → `scaledown_window`, `allow_concurrent_inputs=N` → `@modal.concurrent(max_inputs=N)`. (b) Pre-installed CPU-only torch from `https://download.pytorch.org/whl/cpu` *before* `pip_install_from_requirements`, stopping pip from pulling the full ~5 GB NVIDIA CUDA toolkit. End image is ~1.5 GB lighter and builds in ~1 minute total. RESEND_WEBHOOK_SECRET also set. Charle's-hand remaining: set `NEXT_PUBLIC_API_URL=https://charlenator--autoci-backend-fastapi-app.modal.run` on Vercel + redeploy + send a test inbound email to confirm the scheduled drainer flips a row from `pending` → `processed`. Implementation work for B5 / B6 / B7 / B8 / Sprint C continues separately.
>
