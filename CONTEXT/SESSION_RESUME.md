...
>
> **Update — same evening, fourth pass**: Modal deployment unblocked. `backend/modal_config.py` rewritten end-to-end — single `autoci-secrets` Secret (was 6 phantoms), `pip_install_from_requirements` path corrected, `bge-small-en-v1.5` weights baked into the image at build time so cold starts skip the ~130 MB download, and a second Modal entry-point `drain_inbound_queue` scheduled every 2 minutes calling `inbound_processor.process_all_pending()`. Modal API surface verified against the installed modal 1.4.2. Charle's-hand steps remaining: `cd backend && modal deploy modal_config.py`, set `NEXT_PUBLIC_API_URL` on Vercel to the Modal URL, set `RESEND_WEBHOOK_SECRET` on the Supabase Edge Function, then run the prod smoke test. Implementation work for the rest (B5 / B6 / B7 / B8 / Sprint C) continues separately.
>
