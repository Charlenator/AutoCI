...
| Deploy | Vercel | ⚙️ | S | 8 | Auto-deploys on push; needs `NEXT_PUBLIC_API_URL` set to the Modal URL (left col) and a redeploy |
| Deploy | Modal — backend live | ✅ | M | 8 | URL: `https://charlenator--autoci-backend-fastapi-app.modal.run`. /health 200, /chat/query verified end-to-end (Java TTF returns 83.3 days + B-evidence rows). `drain_inbound_queue` scheduled every 2 min. CPU-only torch pinned via `https://download.pytorch.org/whl/cpu`; image ~1.5 GB lighter. |
| Deploy | Edge Function | ✅ | S | 8 | Inbound webhook deployed; `RESEND_WEBHOOK_SECRET` set |
