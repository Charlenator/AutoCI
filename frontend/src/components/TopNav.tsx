...
const PRIMARY_TABS: NavTab[] = [
  { href: "/", label: "RAG Chat", matchPrefixes: ["/"] },
  { href: "/candidates", label: "Candidate Search", matchPrefixes: ["/candidates"] },
  { href: "/cis", label: "Continuous Improvement Suite", matchPrefixes: ["/cis"] },
];
          ...
          <div className="flex items-center gap-2">
            {SECONDARY_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-gray-500 hover:text-gray-800 px-3 py-2 rounded-md text-xs font-medium"
              >
                {link.label}
              </Link>
            ))}
